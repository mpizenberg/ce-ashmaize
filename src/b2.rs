...
impl VM {
    ...
    pub fn new(rom_digest: &RomDigest, nb_instrs: u32, salt: &[u8]) -> Self {
        ...
        let mut prog_digest = Blake2b512::new();
        prog_digest.update(digests.next().unwrap());

        let mut mem_digest = Blake2b512::new();
    ...
    pub fn post_instructions(&mut self) {
        ...
        self.prog_seed = prog_value.into();
    ...
    match opcode {
        Instr::Op3(operator) => {
            ...
            let result = match operator {
                ...
                Op3::Hash(v) => {
                    assert!(v < 8);
                    // Combine the two 8-byte sources into one 16-byte input array.
...
pub mod argon2 {
    ...
    pub fn hprime(output: &mut [u8], input: &[u8]) {
        if output.len() <= 64 {
            let mut hasher = Blake2bVar::new(output.len()).unwrap();
            hasher.update(&(output.len() as u32).to_le_bytes());
            hasher.update(input);
            let _ = hasher.finalize_variable(output);
            return;
        }
        ...
        let v0_hash = {
            let mut v0_input = vec![0u8; 4 + input.len()];
            v0_input[0..4].copy_from_slice(&(output_len as u32).to_le_bytes());
            v0_input[4..].copy_from_slice(input);
            Blake2b512::digest(&v0_input)
        };
        let mut vi_prev = [0u8; 64];
        vi_prev.copy_from_slice(&v0_hash);
        ...
        let mut hasher = Blake2bVar::new(bytes).unwrap();
        hasher.update(&vi_prev);
        let _ = hasher.finalize_variable(&mut output[pos..pos + bytes]);
    }
}
