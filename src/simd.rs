...
impl VM {
    ...
    pub fn post_instructions(&mut self) {
        ...
        let mixing_value = {
            ...
            hasher.update(prog_value.as_bytes());
            hasher.update(mem_value.as_bytes());
        ...
        Instr::Op3(operator) => {
            ...
            let result = match operator {
                ...
                Op3::Hash(v) => {
                    ...
                    let out = blake2b_simd::blake2b(&input);
...
///
/// ```
/// # use ashmaize::{Rom, RomGenerationType, simd::hash};
...
mod argon2 {
    ...
    pub fn hprime(output: &mut [u8], input: &[u8]) {
        if output.len() <= 64 {
            let mut hasher = Params::new().hash_length(output.len()).to_state();
        ...
        vi_prev.copy_from_slice(v0_hash.as_bytes());
        ...
        while bytes > 64 {
            let vi_hash = blake2b_simd::blake2b(&vi_prev);
            vi_prev.copy_from_slice(vi_hash.as_bytes());
        ...
        output[pos..pos + bytes].copy_from_slice(final_hash.as_bytes());
    }
}
